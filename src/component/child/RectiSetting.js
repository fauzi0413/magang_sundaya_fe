...
function RectiSetting({ sendDataToParent }) {
  ...
  const setMasterFrame = async () => {
    ...
    try {
      ...
      if (error.code === "ECONNABORTED") {
        ...
        error.code === 400 &&
        error.message === "format pcb barcode is incorrect"
      ) {
  ...
  const updateHistory = async () => {
    try {
      ...
      const historyStatus = await res.data.status;
      ...
      if (historyStatus === "success") {
        ...
        console.log("Gagal Update History, retry request history update");
        updateHistory();
      }
    } catch (error) {
      // Handle timeout error
      ...
      if (error.code === "ECONNABORTED") {
        ...
        console.log("Other error occurred: Failed to Connect Recti API");
        ...
        if (countHistory < 6) {
          ...
          alert("Failed to Connect Recti API. Frame Data Can't Update");
        }
      }
    }
  };
